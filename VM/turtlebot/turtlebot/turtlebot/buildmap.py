'''buildmap.py
   ...
   This is the skeleton code for building a map.
...
LFREE     = -0.3         # FIXME.  Set the log odds ratio of detecting freespace
LOCCUPIED = 0.4          # FIXME.  Set the log odds ratio of detecting occupancy
...
class CustomNode(Node):
    ...
    def laserCB(self, msg):
        ...
        # Calculate deltas, scale them based on resolution, set new u,v values
        dx = xc - ORIGIN_X
        dy = yc - ORIGIN_Y
        u0 = int(dx / RESOLUTION)
        v0 = int(dy / RESOLUTION)
        us, vs = u0, v0
        # self.set(u, v, 2.0)

        # Loop through each r, theta combination
        for r, t in zip(ranges, thetas):
            if not np.isfinite(r):
                continue
            if rmin < r < rmax:
                angle = t + thetac
                # Find where the ray hit using trigonometry
                x = xc + r * np.cos(angle)
                y = yc + r * np.sin(angle)
                ...
                ue = int(dx / RESOLUTION)
                ve = int(dy / RESOLUTION)

                if (ue < 0 or ue >= WIDTH or ve < 0 or ve >= HEIGHT):
                    continue
