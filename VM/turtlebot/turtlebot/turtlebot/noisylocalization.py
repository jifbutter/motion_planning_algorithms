'''noisylocalization.py
   ...
   TFBroadcast: odom frame in map frame
'''
...
class CustomNode(Node):
    ...
    def __init__(self, name):
        ...
        # Grab the initial x/y locations from the parameters.
        desc = ParameterDescriptor(type=ParameterType.PARAMETER_DOUBLE)
        self.declare_parameter('x', 0.0, descriptor=desc)
        self.declare_parameter('y', 0.0, descriptor=desc)

        x0 = self.get_parameter('x').get_parameter_value().double_value
        y0 = self.get_parameter('y').get_parameter_value().double_value
